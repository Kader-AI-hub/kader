...
def demo_file_system_tools():
    ...
    try:
        result = write_tool.execute(
            ...
            content="This is content written by WriteFileTool."
        )
        ...
        if isinstance(results, list):
            ...
            for i, result in enumerate(results[:2]):  # Show first 2 results
                ...
                if isinstance(result, dict):
                    title = result.get('title', 'No title') if hasattr(result, 'get') else 'No title'
                elif hasattr(result, '__dict__'):
                    # If it's an object with attributes, try to get title
                    title = getattr(result, 'title', getattr(result, 'get', lambda: 'No title')()) if hasattr(result, 'title') else 'No title'
                else:
                    # For other types, just convert to string
                    title = str(result)[:50]  # Limit length
                print(f"  {i+1}. {str(title)[:50]}...")
        else:
            print(f"Search returned unexpected type: {type(results)}, value: {str(results)[:200]}")
    except Exception as e:
...
"""
    ...
    try:
        ...
        print("RAGSearchTool initialized. In a real scenario, this would search through indexed documents.")
...
def demo_async_operations():
    ...
    async def async_demo():
        ...
        write_task = write_tool.aexecute(
            path="async_test.txt",
            content="This file was written asynchronously."
        )
...
def demo_tool_parameters():
    ...
    for param in read_tool.schema.parameters:
        print(f"  - {param.name}: {param.description} (type: {param.type}, required: {param.required})")
...
def main():
    ...
    print("Note: Some tools may require specific setup (e.g., internet access for web tools,")
