...
FinishReason: TypeAlias = Literal["stop", "length", "tool_calls", "content_filter", "error", None]
...
class ModelPricing:
    ...
    cached_input_cost_per_million: float | None = None  # Cached input cost (if supported)
    ...
    def calculate_cost(self, usage: Usage) -> CostInfo:
        ...
        if self.cached_input_cost_per_million and usage.cached_tokens > 0:
            cached_cost = (usage.cached_tokens / 1_000_000) * self.cached_input_cost_per_million
...
class BaseLLMProvider(ABC):
    ...
    def _merge_config(self, config: ModelConfig | None) -> ModelConfig:
        ...
        return ModelConfig(
            temperature=config.temperature if config.temperature != 1.0 else self._default_config.temperature,
            ...
            frequency_penalty=config.frequency_penalty if config.frequency_penalty != 0.0 else self._default_config.frequency_penalty,
            presence_penalty=config.presence_penalty if config.presence_penalty != 0.0 else self._default_config.presence_penalty,
            ...
            response_format=config.response_format or self._default_config.response_format,
