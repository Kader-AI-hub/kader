...
class TodoToolInput(BaseModel):
    ...
    todo_id: str = Field(..., min_length=1, description="Unique identifier for the todo list")
    items: Optional[list[TodoItem]] = Field(None, description="List of items for create/update")
...
class TodoTool(BaseTool[str]):
        ...
        super().__init__(
            ...
            parameters=[
                ...
                ParameterSchema(
                    ...
                    required=False,
                    properties=[
                        ParameterSchema(name="task", type="string", description="Task description"),
                        ParameterSchema(name="status", type="string", description="Status", enum=["not-started", "in-progress", "completed"], required=False),
                    ]
                ),
                ParameterSchema(
                    name="session_id",
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        try:
            if input_data.action == "create":
                return self._create_todo(session_id, input_data.todo_id, input_data.items)
            elif input_data.action == "read":
                return self._read_todo(session_id, input_data.todo_id)
            elif input_data.action == "update":
                return self._update_todo(session_id, input_data.todo_id, input_data.items)
    ...
    def _create_todo(self, session_id: str, todo_id: str, items: list[TodoItem] | None) -> str:
        ...
        return f"Successfully created todo list '{todo_id}' with {len(items_list)} items."
    ...
    def _update_todo(self, session_id: str, todo_id: str, items: list[TodoItem] | None) -> str:
