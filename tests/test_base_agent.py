...
from kader.providers.base import (
    BaseLLMProvider, Message, ModelConfig, LLMResponse, StreamChunk, Usage
)
...
class MockLLMProvider(BaseLLMProvider):
    ...
    def invoke(self, messages: list[Message], config: ModelConfig | None = None) -> LLMResponse:
    ...
    async def ainvoke(self, messages: list[Message], config: ModelConfig | None = None) -> LLMResponse:
        return self.invoke(messages, config)

    def stream(self, messages: list[Message], config: ModelConfig | None = None) -> Iterator[StreamChunk]:
        self.calls.append(messages)
        content = self._get_next_response()
        # Simulate simple streaming of the content
        yield StreamChunk(content=content, delta=content, finish_reason="stop", usage=Usage(10,10))

    async def astream(self, messages: list[Message], config: ModelConfig | None = None) -> AsyncIterator[StreamChunk]:
        self.calls.append(messages)
        content = self._get_next_response()
        yield StreamChunk(content=content, delta=content, finish_reason="stop", usage=Usage(10,10))
        
    def count_tokens(self, text): return 0
    def estimate_cost(self, usage): return 0
