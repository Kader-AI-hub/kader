...
class TestReadFileTool:
    ...
    def test_execute_with_offset_and_limit(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            tool = ReadFileTool(base_path=Path(temp_dir))
            result = tool.execute(str(test_file.relative_to(temp_dir)), offset=2, limit=2)
...
class TestEditFileTool:
    ...
    def test_execute_replace_all_occurrences(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_content = "Hello universe, universe is beautiful. Say hello to universe."
...
class TestGrepTool:
    ...
    def test_execute_search_with_glob(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (Path(temp_dir) / "test1.py").write_text("def hello(): pass\nprint('hello')")
            (Path(temp_dir) / "test2.txt").write_text("hello world")
            (Path(temp_dir) / "test3.py").write_text("def greet(): pass\nprint('greetings')")
...
class TestSearchInDirectoryTool:
    ...
    def test_execute_search(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_content = "This is a test file containing information about Python programming."
            test_file.write_text(test_content)
...
class TestGetFilesystemTools:
    ...
    def test_get_all_filesystem_tools(self):
        ...
        assert len(tools) == 7  # ReadFileTool, ReadDirectoryTool, WriteFileTool, EditFileTool, GrepTool, GlobTool, SearchInDirectoryTool
