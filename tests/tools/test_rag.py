...
import tempfile
import os
from kader.tools.rag import RAGIndex, RAGSearchTool, DocumentChunk, SearchResult, DEFAULT_EMBEDDING_MODEL
...
class TestRAGIndex:
    ...
    def test_chunk_text(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            long_text = "This is a line.\n" * 50  # 50 lines, each ~15 chars = ~750 chars
    ...
    def test_collect_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (base_path / "test.txt").write_text("just text")  # Should be excluded by default
    ...
    def test_save_and_load(self, mock_pickle, mock_faiss):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_pickle.load.return_value = [DocumentChunk("loaded", "file.py", 1, 10, 0)]
    ...
    def test_build(self, mock_client_class, mock_pickle, mock_faiss):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_file.write_text("def hello():\n    print('Hello, World!')\n" * 20)  # Make it large enough to chunk
    ...
    def test_search(self, mock_client_class, mock_faiss):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create a test file
            test_file = base_path / "test.py"
            test_file.write_text("def search_term():\n    print('This contains the search term')\n" * 5)
            ...
            mock_index_instance.search.return_value = (Mock(), Mock())  # distances, indices
            mock_faiss.IndexFlatL2.return_value = mock_index_instance
